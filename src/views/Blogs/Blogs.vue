<template>
    <div class="p-3">
        <div class="flex justify-between items-center">
            <h2 class="uppercase dark:text-white text-gray-900 text-2xl font-bold">My blogs</h2>
                    ...
                    <form >
                        <div class="grid gap-4 mb-4 sm:grid-cols-1">
                            <div>
                                ...
                                <input v-model="contactInfo.title" type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Blog title">
                            </div>
                            <div>
                                <label for="content" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Content</label>
                                <textarea v-model="contactInfo.content" name="content" id="content" cols="30" rows="10" placeholder="Briefly about my project!" class="w-full h-36 border outline-none border-gray-300 bg-white text-black rounded-lg block p-3"></textarea>
                            </div>
                            <div>
                                <label for="img_link" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Image link</label>
                                <input v-model="contactInfo.img_link" type="text" name="img_link" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="image link" >
                            </div>
                        </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm table-auto">
                        <thead class="text-md text-white font-bold uppercase bg-gray-500 ">
                            <tr class="text-left">
                                <th scope="col" class="px-4 py-3">Blog title</th>
                        ...
                        <tbody class="">
                            <tr v-for="el in computedList" :key="el.id" class="border-b dark:border-gray-700"> 
                                <td class="px-4 py-3 w-[20%]">{{el.title}}</td>
                                <td class="px-4 py-3 w-[60%]">{{el.content}}</td>
                                <td class="px-4 py-3"><img :src="el.img_link" alt="image" class="w-[75%]"></td>
  ...
  <script setup>
    ...
    console.log(contactInfo.title)
    const toggleModal = () => {
        if(modal.value){
            isUpdate.value = false
            contactInfo.title = ''
            contactInfo.content = ''
            contactInfo.img_link = ''
        }
    ...
    const addContact=(evet)=>{
        ...
        const contact = {
            ...
            img_link: contactInfo.img_link,
        }

        useblog.create(contact).then((res)=>{
            if(res.status == 201){
                contactInfo.title = ''
                contactInfo.content = ''
                contactInfo.img_link = ''
    ...
    const modifyContact=(event)=>{
        ...
        useblog.update(id, contact).then((res)=>{
            if(res.status == 200){
                contactInfo.title=''
                contactInfo.content=''
                contactInfo.img_link=''
    ...
    const updateContact = (id)=>{
        ...
        contactInfo.img_link = foundContact[0].img_link
        toggleModal();
    }
