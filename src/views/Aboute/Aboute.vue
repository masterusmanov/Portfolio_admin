<template>
    <div class="p-3">
        <div class="flex justify-between items-center">
            ...
            <h2 class="uppercase dark:text-white text-gray-900 text-2xl font-bold">Social Network</h2>
                ...
                <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                    ...
                    <form >
                        <div class="grid gap-4 mb-4 sm:grid-cols-1">
                            <div>
                                <label for="link" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Social network link</label>
                                <input v-model="contactInfo.social_network" type="text" name="link" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="social_network.com">
                            </div>                            
                            <div>
                                <label for="image_link" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Image Link</label>
                                <input v-model="contactInfo.image_link" type="text" name="image_link" id="image_link" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="imageLink">
                            </div>
            ...
            <div class="bg-white relative shadow-md sm:rounded-lg overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left  table-auto">
                        <thead class="text-md font-bold text-white uppercase bg-gray-500 dark:bg-gray-700 dark:text-gray-400">
                            <tr class="">
                                <th scope="col" class="px-4 py-3">Social network</th>
                                <th scope="col" class="px-4 py-3">Icon</th>
                                <th scope="col" class="px-10 py-3 text-right">Actions</th>
                            </tr>
                        </thead>
                        <tbody class="">
                            <tr v-for="el in computedList" :key="el.id" class="border-b">
                                <td class="px-4 py-3">{{el.social_network}}</td>
                                <td class="px-4 py-3"><img :src="el.image_link" alt="Icon" class="w-[50px]"></td>
...
const addContact=()=>{
    const contact = {
        social_network: contactInfo.social_network,
        image_link: contactInfo.image_link,
    }

    useContact.create(contact).then((res)=>{
        if(res.status == 201){
            contactInfo.social_network = '',
            contactInfo.image_link = ''
            toggleModal()
            updateList();
        }
    }).catch((error)=>{
...
const modifyContact=(event)=>{
    ...
    const contact = {
        link: contactInfo.social_network,
        image_link: contactInfo.image_link
    }
...
const updateContact = (id)=>{
    ...
    contactInfo.social_network = foundContact[0].social_network
    contactInfo.image_link = foundContact[0].image_link
    toggleModal();
}
